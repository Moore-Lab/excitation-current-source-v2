...
Stage 5 is the headline acceptance test (TESTING_PLAN Part 2): per-channel noise
at/below the floor and -- the whole reason the board exists -- **no dependence on
channel/position**, compared directly against the old series-chain data where
...
from host.rtd import sensitivity_ohms_per_c
...
class NoiseStats:
    ...
    def as_row(self) -> Dict[str, float]:
        return {"n": self.n, "mean": self.mean, "std": self.std, "pp": self.pp, "drift": self.drift}
...
def noise_stats(samples: np.ndarray) -> NoiseStats:
    ...
    if n == 0:
        nan = float("nan")
        return NoiseStats(0, nan, nan, nan, nan, nan)
...
def resistance_noise_to_temp(r_noise_ohms: float, r_at_ohms: float, r0_ohms: float) -> float:
